...
export const HeroPresenter: React.FC<HeroPresenterProps> = ({
  onGetStarted,
  onLearnMore,
  isAuthenticated,
  user
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center text-white px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        <p className="text-lg sm:text-xl lg:text-2xl mb-8 text-green-50 leading-relaxed max-w-3xl mx-auto">
          {isAuthenticated ? (
            <>
              ...
              Mari lanjutkan aksi konservasi dan penanaman pohon untuk masa depan bumi yang lebih hijau.
            </>
          ) : (
            <>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          >
            {isAuthenticated ? 'Lihat Posts' : 'Mulai Berpartisipasi'}
          </button>
          ...
          >
            {isAuthenticated ? 'Tentang Platform' : 'Pelajari Lebih Lanjut'}
          </button>
        </div>
      </div>
